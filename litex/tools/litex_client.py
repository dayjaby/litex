...
def main():
    ...
    if args.ident:
        dump_identifier(port=port, csr_csv=args.csr_csv, debug=args.debug)

    if args.regs:
        dump_registers(filter=args.filter, port=port, csr_csv=args.csr_csv, debug=args.debug)

    if args.read:
        read_memory(port=port, addr=int(args.read, 0))

    if args.write:
        write_memory(addr=int(args.write[0], 0), data=int(args.write[1], 0), port=port, csr_csv=args.csr_csv, debug=args.debug)

    if args.read:
        read_memory(addr=int(args.read, 0), port=port, csr_csv=args.csr_csv, debug=args.debug)
